...
const DraftSurveyValeursFinal = () => {
    const { survey, updateFinal } = useDraftSurvey();
    const navigate = useNavigate();
    
    const [localData, setLocalData] = useState(survey.final);

    const handleNext = () => {
        updateFinal(localData);
        navigate('/draft-survey/calculation');
    };
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 pt-20 px-4">
            <div className="max-w-5xl mx-auto">
                <div className="flex items-center justify-between mb-8">
                    <button 
                        onClick={() => navigate('/draft-survey/displacement')}
                    ...
                    <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Valeurs Finales</h1>
                    <div className="w-20" />
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-8 shadow-xl">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-1">
                                <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Autres</label>
                        ...
                        <div className="mt-8 pt-8 border-t border-slate-100 dark:border-slate-700">
                             <div className="space-y-2">
                                <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">Densité de l'eau (Final)</label>
                                <input 
                                    type="number"
                                    step="0.0001"
                                    value={localData.density}
                                    onChange={(e) => setLocalData(prev => ({ ...prev, density: parseFloat(e.target.value) || 1.025 }))}
                                    className="w-full px-4 py-3 border border-slate-200 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-indigo-500 dark:bg-slate-700 dark:text-white font-mono"
                                />
                            </div>
                        </div>
                    </div>
                ...
                <div className="mt-8 pt-8 flex justify-between">
                    ...
                    <button 
                        onClick={handleNext}
                        className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-10 py-4 rounded-xl font-bold shadow-lg hover:shadow-xl transition-all group"
                    >
                        Suivant: Calcul Final
...
export default DraftSurveyValeursFinal;